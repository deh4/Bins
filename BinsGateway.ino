...
IPAddress myIp (192, 168, 0, 66);  // Configure your static ip-address here    COMPILE ERROR HERE? Use Arduino IDE 1.5.7 or later!

// The MAC address can be anything you want but should be unique on your network.
// Newer boards have a MAC address printed on the underside of the PCB, which you can (optionally) use.
// Note that most of the Ardunio examples use  "DEAD BEEF FEED" for the MAC address.
byte mac[] = { 0xDE, 0xAD, 0xBE, 0xEF, 0xFE, 0xED };  // DEAD BEEF FEED
char servers[] = "www.dehakuran.com";
...
{ 
  ...
  gw.begin(RF24_PA_LEVEL_GW, RF24_CHANNEL, RF24_DATARATE, PushData);
...
void PushData(char *writeBuffer)
{
    // if an incoming client connects, there will be
  // bytes available to read via the client object
  //= server.available();
  if (cliente.connect(IPAddress(188,124,10,36), 80)) {
    Serial.println("-> Connected");
    // Make a HTTP request:
    cliente.print( "GET /vcard/rogar/submit.php?");
    cliente.print("StationName=");
    cliente.print( "Darlama" );
    cliente.print("&&");
    cliente.print("level=");
    cliente.print( "27" );
    cliente.println( " HTTP/1.1");
    cliente.print( "Host: " );
    cliente.println(servers);
